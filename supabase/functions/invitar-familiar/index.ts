// =====================================================================
// invitar-familiar — edge function (proyecto sydzfwwiruxqaxojymdz).
//
// Invita a un FAMILIAR (rol 'secundario') a la cuenta de un cliente, con
// UN SOLO correo. Misma mecánica que invitar-cliente, pero:
//   - quien llama debe ser el PRINCIPAL de una cuenta (no un admin),
//   - el cliente_id sale de SU fila en usuarios_cliente (nunca del body),
//   - tope de 3 familiares por cuenta,
//   - la invitación se UPSERT con rol 'secundario'.
//
// Reusa de invitar-cliente, sin tocar una línea: imports + constantes de
// cabecera, buildCors(), esUsuarioYaExiste() y enviarCorreo() (incluido
// todo el HTML del correo). La service_role y la API key de Brevo viven
// solo acá, como variables de entorno — nunca tocan el cliente.
// =====================================================================
...
Deno.serve(async (req: Request): Promise<Response> => {
    ...
    try {
        ...
        // 2 — Verificar que el caller es el PRINCIPAL de una cuenta.
        //     El cliente_id sale de SU fila, nunca del body.
        const { data: filaCaller, error: callerErr } = await admin
            .from('usuarios_cliente')
            .select('cliente_id, rol')
            .eq('auth_user_id', caller.id)
            .maybeSingle();
        if (callerErr) {
            return json({ ok: false, error: 'No se pudo verificar la cuenta' }, 500);
        }
        if (!filaCaller || filaCaller.rol !== 'principal') {
            return json({ ok: false, error: 'Solo el titular de la cuenta puede invitar a un familiar' }, 403);
        }
        const clienteId = filaCaller.cliente_id;

        // 3 — Validar el body (solo email + nombre del familiar).
        const body = await req.json().catch(() => null);
        const email = String(body?.email ?? '').trim().toLowerCase();
        const nombre = String(body?.nombre ?? '').trim();
        if (!EMAIL_RE.test(email)) return json({ ok: false, error: 'Email inválido' }, 400);
        if (!nombre) return json({ ok: false, error: 'Falta el nombre del familiar' }, 400);

        // 4 — Tope de 3 familiares. El RPC excluye al propio invitado, así
        //     que reinvitar a un familiar existente no consume cupo.
        const { data: nSecundarios, error: topeErr } = await admin
            .rpc('contar_secundarios', { p_cliente_id: clienteId, p_email: email });
        if (topeErr) {
            return json({ ok: false, error: 'No se pudo verificar el límite de familiares' }, 500);
        }
        if ((nSecundarios ?? 0) >= 3) {
            return json({ ok: false, error: 'Has alcanzado el máximo de 3 familiares en tu cuenta' }, 409);
        }

        // 5 — UPSERT de la invitación con rol 'secundario'.
        ...
        const { error: upsertErr } = await admin
            .from('invitaciones_pendientes')
            .upsert(
                { email, cliente_id: clienteId, nombre, rol: 'secundario', invitado_por: caller.id },
                { onConflict: 'email' },
            );
        if (upsertErr) {
        ...
        // 6 — Crear el usuario (no dispara correo de GoTrue).
        ...
        // 7 — Generar el código de 6 dígitos (sin correo de GoTrue).
        ...
        // 8 — Mandar el correo con el código.
        try {
